...
export const ProjectsModule = ({ onShowToast }) => {
  ...
  const openReturnReasonModal = async (project) => {
    ...
    try {
      // Consultar todas las aprobaciones y filtrar/ordenar en frontend
      const baseUrl = import.meta.env.VITE_SUPABASE_URL
      const url = `${baseUrl}/rest/v1/aprobacion_proyecto?select=motivo_devolucion,fecha_aprobacion,id_aprobacion,estado_asignado,id_proyecto`
      ...
      filtered.sort((a, b) => {
        if (a.fecha_aprobacion !== b.fecha_aprobacion) {
          return (b.fecha_aprobacion || "").localeCompare(a.fecha_aprobacion || "")
        }
        return (b.id_aprobacion || 0) - (a.id_aprobacion || 0)
      })
      const approvalFound = filtered[0]
